...
# desk_lamps = ['3d/desk1.obj', '3d/desk3.obj', '3d/desk2.obj','3d/desk0.obj', '3d/desk4.obj' ]
...
#     vertical_view = $container.width() < $container.height()

#     step = if vertical_view then 30 else 40#(width/35)
#     offset = if vertical_view then step/2 else 0
...
#             if vertical_view and (c != 2) and (c !=3)
...
#             group.userData.rx = if rx? then rx else 0
#             group.userData.rz = if rz? then rz else 0
...
#         # y += (position.x-.5)*2 * (Math.PI/8) *(1-scroll_percent)

#         x = -s * group.userData.random_noise_x
#         # group.rotation.z = (position.y-.5)*2 * (Math.PI/8)# * (1-scroll_percent)
