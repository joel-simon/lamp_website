  ...
  $('.scrollcontainer').scroll (event) ->
    $lamps.each () ->
      if $(@).hasClass 'viewer'
        ...
        viewer.rotation.z = (((percX - 0.50)/4) * Math.PI)
...
scrollEnd = () ->
  ...
  $('.lamp').each () ->
    ...
    if Math.abs(sx-x) < minX
      minX = Math.abs(sx-x)
      derp = $(@)

  scrollLeft = $('.scrollcontainer').scrollLeft() + sx-(derp.offset().left + derp.width()/2)
...
$ ->
  ...
  #   $(@).data 'scrollTimer', (setTimeout scrollEnd, 250)
